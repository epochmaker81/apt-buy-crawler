# <<< ConnectionError 해결 및 2개월 조회/자동 누적 최종 버전 main.py >>>
...
def fetch_data_for_region(session, lawd_cd, deal_ymd, service_key):
    ...
    # --- [해결 방안 2] 프록시 설정 로직 추가 ---
    # GitHub Secret에 PROXY_URL이 설정된 경우에만 프록시를 사용합니다.
    ...
    for attempt in range(3):
        try:
            # session.get 호출 시 proxies 인자 추가
...
def main():
    ...
    for month in MONTHS_TO_FETCH:
        ...
        for i, code in enumerate(lawd_codes):
            ...
            # --- [해결 방안 1] API 서버 부하 감소를 위한 지연 시간 증가 ---
            time.sleep(0.5)
